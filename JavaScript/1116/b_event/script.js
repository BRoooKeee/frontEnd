...
1. 이벤트 객체
  ...
  예) 키보드 이벤트 - 어떤 키가 눌렸는지에 대한 정보
  : event, evt, e 등과 같은 이름을 명명된 매개변수로 전달
*/
  ...
  /*
    이벤트 객체 실습
      ...
      - 각각의 타일을 클릭했을 떄 클리된 타일의 배경색이 변경되도록 지정
  */
  function bgChange() {
...
/*
  2. 기본 행동 방지
    : 텍스트 필드가 비워졌는지를 검사하는 onsubmit 이벤트 핸들러 구현
    : 비었다면 이벤트 객체에 있는 양식 제출을 멈추도로 지정
    : 양식 아래에 있는 단락에 에러 메시지를 표시
*/
  const form = document.querySelector('form');
  const fname = document.getElementById('fname');
  const email = document.getElementById('email');
  const para = docu.querySelector('p');
...
3. 이벤트 버블링과 캡쳐
  ...
  - 이벤트 버블링 (내부 > 외부 요소)
    ...
    예) HTML 페이지에서 버튼 요소가 클릭되면 버튼 요소부터 시작해서 그의 부모 요소들을 거쳐서 최상위 요소까지 전달
