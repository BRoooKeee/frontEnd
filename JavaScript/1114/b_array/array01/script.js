...
// 배열의 정의
// : 여러 데이터 요소를 순서대로 저장하는 자료구조
// : 같은 타입이 아닌 여러 값들(여러개의 변수)을 담을 수 있는 리스트 형태의 객체
...
let mixedTypes = [1, 'two', false, [5, 6]];   // 다른 데이터 타입
...
let arrayTwo = new Array('2');
console.log(arrayLengthTwo);  // 길이가 2인 빈 배열
console.log(arrayTwo);  // '2'라는 하나의 요소를 포함하는 배열을 생성
...
cars[7] = 'kia';
console.log(cars);  //['bmw', 'audi', 'hyundai', empty × 4, 'kia']
...
console.log(cars);
cars.unshift('newBMW'); // 첫번째 자리에 추가
console.log(cars);
...
//    - end (선택)
//         : 생략 시 배열의 끝까지 추출
//         : end 번호 바로 앞의 인덱스까지만 수행
let slicedCars = cars.slice(1, 4); // 인덱스 번호 1, 2, 3이 추출
console.log(slicedCars); // ['audi', 'hyundai', empty]
console.log(cars); // ['newBMW', 'audi', 'hyundai', empty × 4, 'kia'] 기존에서 변경X


// - splice()
//  : 배열의 기존 요소를 삭제 또는 교체 | 새 요소를 추가여 배열의 내용을 변경
//    배열.splice(start)  >> 변경을 시작할 인덱스 번호, 해당 시작 인덱스부터 요소가 제거되거나 새로운 요소가 삽입
//    배열.splice(start, deleteCount)   >>  제거할 요소의 개수, 생략 시 start인덱스부터 배열의 끝까지 모든 요소가 제거
//    배열.splice(start, deleteCount, item1, item2, ...);   >>  배열에 삽입될 요소, start인덱스 이후부터 삽입, 생략시 삽입되지 X
console.log(cars);    // ['newBMW', 'audi', 'hyundai', empty × 4, 'kia']
// 인덱스 3번부터 4개의 요소를 제거하고 'jeep', 'volvo' 삽입
...
// join메소드 안의 문자열 값을 기준으로 배열릐 요솓르이 문자열로 반환
let carString = combinedCars.join(' ');
console.log(carString);

// split() 메소드 (문자열의 메소드)
// 문자열로 이루어진 데이터를 정제하여 배열의 형태로 나타낼 때 사용
let myData = 'Seoul, Busan, Daejoen';
