...
- reduce : 배열의 각 요소에 대해 함수를 적용하여 단일 값 생성
    ...
    - accumulator : 이전 리듀서의 호출의 반환 값, 각 호출에서 축적되는 값
    - currentValue : 현재 처리 중인 배열의 요소
    - currentIndex : (선택) 현재 처리중인 배열 요소의 인덱스
    - array : (선택) reduce가 호출된 배열
    - initialValue : (선택) 작성을 하지 않은 경우 배열의 첫 번째 요소가 초기값으로 지정
    >> 배열의 모든 숫자를 더하는 합계 계산
*/
let numbers = [1, 2, 3, 4];
let sum = numbers.reduce((accumulator, 
  currentValue) => 
    accumulator + currentValue,
    0);
    console.log(sum);

// reduce함수를 사용하여 모든 차량 브랜드 이름을 연결, 데이터 사이에는 ', '
...
1. sort(compareFunction)
  : 배열 요소 정렬
  - compareFunction 
    : 값이 생략될 경우, 배열릐 요소들은 문자열로 취급, 유니코드 값 순서대로 정렬
    : 해당 함수가 리턴하는 값이
...
/*
3. toString, toLocalString (지역 설정에 맞는 문자열로 변환-날짜)
  : 배열을 문자열로 변환
*/
