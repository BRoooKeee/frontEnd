...
1. 클래스(Class)
  - 클래스 정의
    ...
    : 클래스명은 대문자로 시작
  - 클래스 특징
    1) 캡슐화 : 데이터(속성)와 데이터를 처리하는 함수(메서드)를 하나로 묶는 것
    2) 추상화 : 복잡한 세부사항X, 필수적인 부분만 강조하는 것
    3) 상속 : 한 클래스의 속성과 메서드를 다른 클래스가 상속받을 수 있는 것
    4) 다형성 : 하나의 인터페이스를 통해 다른 클래스의 인스턴스를 다룰 수 있는 것
    class ClassName {
...
2. 객체 (Object)
  ...
  - 객체 특징
    1) 상태와 행동: 객체는 속성(상태)과 메서드(행동)를 갖는 것
    2) 독립성: 각 객체는 독립적인 성격을 가지며 다른 객체와 상호 작용
...
//  : 속성 'title'은 이니셜라이저가 없고 생성자에 할당되어 있지 않습니다.
