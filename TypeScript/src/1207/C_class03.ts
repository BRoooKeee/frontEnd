...
! 접근 제어자, 제한자(Access Modifiers)
  ...
  캡슐화
    : 클래스의 내부 데이터를 외부에서 직접 접근하는 것을 제한하여 데이터의 안정성을 보장하는 방식
  ...
  ? 종류
    ...
    - protected
      ...
      : 기본적으로 하위클래스는 상위 클래스의 멤버에 접근 가능하지만 해당 멤버가 private이라면 접근 X
...
let employee1 = new Employee('kim', 500, 'IT');
console.log(employee1.name); //public > 외부접근가능
...
//  : 해당 매개변수는 같은 이르므이 속성(멤버변수)으로 선언, 해당 매개변수의 인자는 암묵적으로 인스턴스에 할당
class User {
...
let lee = new User('lee', '1234');
console.log(`${lee.id}`);
// console.log(`${lee.password}`); //password는 private 속성
}