...
! 생성자(Constructor)
  ...
  ? 생성자 함수 사용법
    : constructor 키워들르 사용하여 클래스 내부에 정의
    : 인자를 받아서 클래스의 프로퍼티를 초기화

  ? 특징
    : 클래스는 단 하나의 생성자만 가질 수 있다.
    : 반환값 X, 자신을 new 키워드와 함께 호출하는 대상(클래스)의 새로운 인스턴스를 암시적으로 반환
    : 필수적 요소X, 생성자가 없는 경우 TS는 빈 생성자를 자동으로 추가
*/
class Person {
  // 멤버 변수
  name: string;
...
! this 키워드
  ...
  ? 사용 시 주의점
    : 메소드나 생성자 내에서 클래스의 멤버에 접근할 때 this 사용
    : 화살표 함수 내에서 사용
    : - 자신만의 this 바인딩을 생성하지 X
    : - 해당 함수가 선언되 범위의 this 사용
