...
! 추상 클래스와 추상 메소드
  ? 정의
    ...
    2. 추상 메소드
      ...
      : 파생 클래스가 반드시 구현해야하는 메소드의 틀을 제공
      : 추상 클래스 내에서 abstract 키워드를 사용하여 선언
*/
abstract class Animal {
  abstract makeSound(): void; //추상 메소드

  // 추상클래스 내에서는 반드시 한 개 이상의 추상 메소드가 정의되어야함
...
class Dog extends Animal {
  // 추상 클래스를 상속받은 클래스에서는 반드시 추상 메소드를 구현해야함
  makeSound(): void {
    console.log('Wof');
  }
}
...
/*
! 인터페이스 구현(Implementing Interaces)
  ? 정의
