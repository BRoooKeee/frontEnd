...
//? 회원 관리 프로그램
//  : 추가(addMember), 검색(findMember), 제거(removeMember), 활성 회원만 필터링(filterActiveMembers)하는 기능을 제공
...
class Membership<T extends Member> {
  ...
  // 활성 상태인(active가 true) 회원만을 필터링하여 반환하는 메소드
...
membership.addMember(new Member('lee', 39));
membership.addMember(new Member('kim', 31));
membership.addMember(new Member('park', 17));


// 'lee'라는 이름을 가진 회원을 출력
console.log(membership.findMember('lee'));

// 활성상태인 회원드란 필터링하여 출력
let activeMembers = membership.filterActiveMembers();
console.log(activeMembers);

// 'kim'이라는 이름을 가진 회원 제거
membership.removeMember('kim');

// 'kim'이라는 이름을 가진 회원 출력
console.log(membership.findMember('kim'));