...
*/
  function generic<T>(arg: T): T {
    // T는 타입 변수 (단일 타입 변수)
    //  : 해당 타입 변수는 입력 인수의 타입을 T로 지정하고 함수의 반환 타입을 T로 지정
    return arg;
  }
// 제네릭으로 지정된 함수 사용
...
//   - 제네릭 함수
      ...
      let resultFunc = genericFunc<boolean>([true, false, true]); //3
      console.log(resultFunc); //[ true, false, true ]

//   - 제네릭 인터페이스  :  타입 매개변수<T>를 가지는 인터페이스
...
//     : 다양한 타입의 속성이나 메소드를 지정 가능
      ...
      console.log(myGenericVar.add(5, 2));
