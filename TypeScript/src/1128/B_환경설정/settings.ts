...
! Node.js란?
  ...
  ? Node.js 설치 전 삭제
  - 윈도우(Windows)
    : 제어판 > 프로그램 추가/제거 > Node.js 삭제
    : 아래 경로에 해당하는 디렉터리 삭제
      C://Program Files/Nodejs
      C://Program Files(x83)/Nodejs
...
! 2. npm이란?
  ...
  : NPM을 이용하면 자바스크립트 라이브러리르 쉽게 설치하고 관리
  ...
  ? npm 기본 명령어
    1) npm init
      : 새로운 Node.js 프로젝트를 시작하고 기본값으로 package.json 파일을 생성
      : -y 옵션 추가 -> 질문 없이 기본값으로 package.json 파일 생성
    2) npm install
      : package.json 파일에 명시된 모든 의존성을 설치
      : 특정 패키지 설치 시 -> npm install 패키지이름
      : -D | --save-dev 옵션 추가 -> 개발 의존성으로 패키지를 설치
    3) npm uninstall
      : 패키지를 제거
      : 특정 패키지 제거 시 -> npm uninstall 패키지이름
*/
...
! 4. tsc란?
  ...
  ? tsc의 옵션
    ...
    target : 컴파일된 js버전을 지정
    module : 사용할 모듈 시스템을 지정
    strict : 모든 엄격한 타입-체크 옵션을 활서오하
...
! 5. 타입스크립트 패키지 설치
  ...
  ? npm 초기화
    npm init -y
    : 새로운 Node.js 프로젝트를 시작하고 기본값을 package.json 파일을 생성
    : package.json
  ...
  ? npm을 이용한 타입스크립트 패키지 설치
    ...
    - 전역 설치
      npm install -g typescript
      : 시스템 전체에서 타입스크립트를 사용
      : 컴퓨터의 어느 위치에서든 타입스크립트 컴파일러(tsc)를 사용가능
  ...
  ? tsconfig.json 생성
    ...
    : 예) 출력 디렉토리 위치, 사용할 모듈 시스템, 지원 ECMAScript 버전 등을 관리
    - tsconfig.json 옵션 정리
      1) compilerOptions : 컴파일러 옵션을 설정하는 객체입니다. 이 객체에는 여러 하위 옵션이 있습니다.
      2) target : 컴파일된 코드가 실행될 ECMAScript 버전을 지정합니다. 예를 들어, "ES5", "ES2015" 등을 지정할 수 있습니다.
      3) module : 사용할 모듈 시스템을 지정합니다. 예를 들어, "CommonJS", "ES2015", "AMD" 등을 지정할 수 있습니다.
      4) outDir : 컴파일된 자바스크립트 파일이 위치할 티렉토리를 지정합니다.
      5) strict : 모든 엄격한 타입-체킹 옵션을 활성화 합니다.
      6) include : 컴파일에 포함될 파일 또는 디렉토리의 리스트입니다.
      7) exclude : 컴파일에서 제외될 파일 또는 디렉토리의 리스트입니다.
